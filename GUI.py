...
# UI--Logic分离
main_ui, _ = loadUiType(f'{root}/UI/main.ui')
...
class MainGui(QMainWindow, main_ui):
    ...
    def __init__(self):
        ...
        # 设置图标
        icon = QtGui.QIcon()
        icon.addPixmap(QtGui.QPixmap(f"{root}/icon/app.png"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        self.setWindowIcon(icon)

        icon1 = QtGui.QIcon()
        icon1.addPixmap(QtGui.QPixmap(f"{root}/icon/dirs.png"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        self.pushButton_img.setIcon(icon1)

        icon2 = QtGui.QIcon()
        icon2.addPixmap(QtGui.QPixmap(f"{root}/icon/dir.png"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        self.pushButton_dir.setIcon(icon2)

        icon3 = QtGui.QIcon()
        icon3.addPixmap(QtGui.QPixmap(f"{root}/icon/shipinwenjian.png"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        self.pushButton_video.setIcon(icon3)

        icon4 = QtGui.QIcon()
        icon4.addPixmap(QtGui.QPixmap(f"{root}/icon/shexiangtou.png"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        self.pushButton_cap.setIcon(icon4)

        icon5 = QtGui.QIcon()
        icon5.addPixmap(QtGui.QPixmap(f"{root}/icon/data.png"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        self.pushButton_data.setIcon(icon5)

        icon6 = QtGui.QIcon()
        icon6.addPixmap(QtGui.QPixmap(f"{root}/icon/weights.png"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        self.pushButton_weights.setIcon(icon6)

        palette = self.palette()
        palette.setBrush(QPalette.Background, QBrush(QPixmap(f"{root}/icon/backgroung.png")))  # 设置背景图片
        self.setPalette(palette)

        self.label_4.setPixmap(QtGui.QPixmap(f"{root}/icon/IOU.png"))
        self.label_5.setPixmap(QtGui.QPixmap(f"{root}/icon/zhixindu.png"))
        self.label_8.setPixmap(QtGui.QPixmap(f"{root}/icon/NVIDIA.png"))
        self.label_11.setPixmap(QtGui.QPixmap(f"{root}/icon/yongshi.png"))
        self.label_14.setPixmap(QtGui.QPixmap(f"{root}/icon/zhixindu.png"))
        self.label_15.setPixmap(QtGui.QPixmap(f"{root}/icon/leibie.png"))
        self.label_16.setPixmap(QtGui.QPixmap(f"{root}/icon/half.png"))
        self.label_17.setPixmap(QtGui.QPixmap(f"{root}/icon/weizhi.png"))
        self.label_22.setPixmap(QtGui.QPixmap(f"{root}/icon/imgs.png"))
        self.label_25.setPixmap(QtGui.QPixmap(f"{root}/icon/suoyoumubiao.png"))

        self.label_size = [self.label_img.size().width(), self.label_img.size().height()].copy()
    ...
    def ShowSource(self, img):
        ...
        flag = False
        if image_width > label_width and image_height <= label_height:
            scale_factor = label_width / image_width
            new_height = image_height * scale_factor
            img = cv2.resize(img, (label_width, int(new_height)))
        elif image_height > label_height and image_width <= label_width:
            scale_factor = label_height / image_height
            new_width = image_width * scale_factor
            img = cv2.resize(img, (int(new_width), label_height))
        elif image_width < label_width and image_height < label_height:
            flag = True
        ...
        if not flag:
            self.label_img.setPixmap(pixmap.scaled(self.label_img.size(), Qt.KeepAspectRatio, Qt.SmoothTransformation))
        else:
